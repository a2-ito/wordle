...
const WORDS = [
  ...
  "broad","brown","build","buyer","cable","calif","carry","catch","cause","chain",
  ...
  "happy","harry","heart","heavy","hence","hotel","house","human","ideal","image",
  ...
  "react","ready","refer","right","rival","river","robot","rough","round","route",

  "scale","scene","scope","score","sense","serve","seven","shall","shape","share",
  "sharp","sheet","shelf","shell","shift","shirt","shock","shoot","short","shown",
  "sight","since","skill","sleep","small","smart","smile","smoke","solid","solve",
  ...
  "tough","tower","track","trade","train","treat","trend","trial","tried","trust",
...
export default function Board() {
  ...
  function evaluateGuess(guess: string[], answer: string): TileStatus[] {
    ...
    guess.forEach((char, i) => {
      ...
      const index = answerChars.findIndex(
        (c, j) => c === char && !used[j]
      );
      if (index !== -1) {
        result[i] = "present";
  ...
  function handleKey(key: string) {
    ...
    if (key === "enter") {
      ...
      const priority = { correct: 3, present: 2, absent: 1, unused: 0 } as const;
